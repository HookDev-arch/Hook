...
- Update `Hook-tl` in accordance to upstream telethon v1
- Update `Hook-pyro` in accordance to upstream pyrogram
...
- Drop Okteto support
- Show Hook platform and version in Telegram sessions list
...
- Fix `Hook.types.CoreOverwriteError`
...
- Add `Hook.validators.MultiChoice`
- Add `utils.get_args_html` to get arguments of command with HTML
- Add switch to mute @BotFather only once in Hook inline
...
- Change inline query placeholder to `user@Hook:~$` + legacy migration
...
- Create new type :obj:`Hook.tl_cache.CustomTelegramClient` to avoid monkeypatching
- Add `ttl` param for :method:`Hook.utils.asset_channel`
- Add support for custom branches (e.g. for beta testers and users, who rolled back)
- Fix automatic modules reactions
- Fix :method:`Hook.inline.utils.Utils._find_caller_sec_map`
- Fix the targeted security rules without time limit
- Require Hook-TL >= 1.24.9
...
- Publish Hook telethon fork and migrate to it in requirements, thereby fixing the deployment error on Heroku
...
- Fix `utils.find_caller` for :method:`Hook.inline.utils.Utils._find_caller_sec_map`
- Fix `.eval`
...
- Add platform-specific Hook emojis to premium users
...
- Move `Hook._types` to `Hook.types` with legacy support
- Remake all core modules to decorators
- Force custom Hook telethon installation with 144 layer support
...
- Remove suffix `Hook` if user specified it somewhere below
...
- Add `client.force_get_entity` to bypass Hook Cacher
...
- Add banner with Hook installation status
...
- Return :obj:`InlineMessage` in `Hook.inline.gallery.Gallery.gallery`
...
- Add automatic Hook folder processing
- Add avatars to all official repo modules, which require asset chats (and add them to Hook folder)
...
- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/hikariatama/Hook/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes
