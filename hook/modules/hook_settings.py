...
class HookSettingsMod(loader.Module):
    ...
    async def _uninstall(self, call: InlineCall):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"hook-logs", "silent-tags"}
                )
                or (
        ...
        if any(folder.title == "hook" for folder in folders):
            folder_id = max(
                folders,
                key=lambda x: x.id,
            ).id
    ...
    async def weburl(self, message: Message, force: bool = False):
        if "LAVHOST" in os.environ:
            form = await self.inline.form(
                ...
                reply_markup={
                    ...
                    "url": await main.hook.web.get_url(proxy_pass=False),
                },
        ...
        if not main.hook.web:
            main.hook.web = core.Web(
                data_root=main.BASE_DIR,
                api_token=main.hook.api_token,
                proxy=main.hook.proxy,
                connection=main.hook.conn,
            )
            await main.hook.web.add_loader(self._client, self.allmodules, self._db)
            await main.hook.web.start_if_ready(
                len(self.allclients),
                main.hook.arguments.port,
                proxy_pass=main.hook.arguments.proxy_pass,
            )
        ...
        url = await main.hook.web.get_url(proxy_pass=True)
