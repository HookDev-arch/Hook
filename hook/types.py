...
class Module:
    ...
    async def _decline(
        ...
        self._db.set(
            "hook.main",
            "declined_joins",
            list(set(self._db.get("hook.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?hikka_min", code):
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Hook version {'{}.{}.{}'.format(*ver)}+"
                    )
                )
        ...
        if (
            ...
            and self._db.get("hook.main", "stats", True)
            and url is not None
